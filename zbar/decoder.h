...
static __inline unsigned calc_s(const zbar_decoder_t *dcode,
  ...
  unsigned s = 0;
  while (n--) s += get_width(dcode, offset++);
  return (s);
}
 ...
 *   - n should be > 3
 */
static __inline int decode_e(unsigned e, unsigned s, unsigned n) {
...
static __inline unsigned decode_sort3(zbar_decoder_t *dcode, int i0) {
  ...
  if (w0 < w2) {
    if (w2 < w4) return ((i0 << 8) | ((i0 + 2) << 4) | (i0 + 4));
    if (w0 < w4) return ((i0 << 8) | ((i0 + 4) << 4) | (i0 + 2));
    return (((i0 + 4) << 8) | (i0 << 4) | (i0 + 2));
  }
  if (w4 < w2) return (((i0 + 4) << 8) | ((i0 + 2) << 4) | i0);
  if (w0 < w4) return (((i0 + 2) << 8) | (i0 << 4) | (i0 + 4));
...
static __inline char size_buf(zbar_decoder_t *dcode, unsigned len) {
  unsigned char *buf;
  if (len <= BUFFER_MIN) return (0);
  if (len < dcode->buf_alloc) /* FIXME size reduction heuristic? */
    return (0);
