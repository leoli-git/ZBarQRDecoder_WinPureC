...
#ifdef ENABLE_DATABAR
  if (dcode->databar.segs) zbar_free(dcode->databar.segs);
#endif
  if (dcode->buf) zbar_free(dcode->buf);
  zbar_free(dcode);
}
...
zbar_decoder_handler_t *zbar_decoder_set_handler(
    zbar_decoder_t *dcode, zbar_decoder_handler_t *handler) {
...
zbar_symbol_type_t zbar_decode_width(zbar_decoder_t *dcode, unsigned w) {
...
#endif
#ifdef ENABLE_EAN
  if ((dcode->ean.enable) && (tmp = _zbar_decode_ean(dcode))) sym = tmp;
#endif
#ifdef ENABLE_CODE39
  ...
  if (sym) {
    ...
    if (dcode->handler) dcode->handler(dcode);
  }
  return (sym);
}

static __inline const unsigned int *decoder_get_configp(
    const zbar_decoder_t *dcode, zbar_symbol_type_t sym) {
...
static __inline int decoder_set_config_bool(zbar_decoder_t *dcode,
  ...
  if (!config || cfg >= ZBAR_CFG_NUM) return (1);
...
const char *_zbar_decoder_buf_dump(unsigned char *buf, unsigned int buflen) {
  ...
  if (!decoder_dump || dumplen > decoder_dumplen) {
    if (decoder_dump) zbar_free(decoder_dump);
