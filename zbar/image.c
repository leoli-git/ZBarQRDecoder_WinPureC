...
void zbar_image_set_symbols(zbar_image_t *img, const zbar_symbol_set_t *syms) {
  if (syms) zbar_symbol_set_ref(syms, 1);
  if (img->syms) zbar_symbol_set_ref(img->syms, -1);
...
int zbar_image_write(const zbar_image_t *img, const char *filebase) {
  ...
  if ((img->format & 0xff) >= ' ')
    ...
    n = snprintf(filename, len, "%s.%08lx.zimg", filebase, img->format);
  assert(n < len - 1);
  filename[len - 1] = '\0';
...
int zbar_image_write_png(const zbar_image_t *img, const char *filename) {
  ...
  rows[0] = img->data;
  int y;
  for (y = 1; y < img->height; y++) rows[y] = rows[y - 1] + img->width;
