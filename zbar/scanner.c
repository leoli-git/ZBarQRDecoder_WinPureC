...
void zbar_scanner_destroy(zbar_scanner_t *scn) { zbar_free(scn); }
...
zbar_symbol_type_t zbar_scanner_reset(zbar_scanner_t *scn) {
  ...
  if (scn->decoder) zbar_decoder_reset(scn->decoder);
  return (ZBAR_NONE);
}
...
static __inline unsigned calc_thresh(zbar_scanner_t *scn) {
  ...
  if (thresh > t) {
    thresh -= t;
    if (thresh > scn->y1_min_thresh) return (thresh);
  }
  scn->y1_thresh = scn->y1_min_thresh;
  return (scn->y1_min_thresh);
}

static __inline zbar_symbol_type_t process_edge(zbar_scanner_t *scn, int y1) {
  if (!scn->y1_sign)
  ...
  /* pass to decoder */
  if (scn->decoder) return (zbar_decode_width(scn->decoder, scn->width));
  return (ZBAR_PARTIAL);
}

__inline zbar_symbol_type_t zbar_scanner_flush(zbar_scanner_t *scn) {
  unsigned x;
  if (!scn->y1_sign) return (ZBAR_NONE);
  ...
  if (scn->decoder) return (zbar_decode_width(scn->decoder, 0));
  return (ZBAR_PARTIAL);
}
...
zbar_symbol_type_t zbar_scanner_new_scan(zbar_scanner_t *scn) {
  ...
  while (scn->y1_sign) {
    ...
    if (tmp < 0 || tmp > edge) edge = tmp;
  }

  /* reset scanner and associated decoder */
  memset(&scn->x, 0, sizeof(zbar_scanner_t) - offsetof(zbar_scanner_t, x));
  scn->y1_thresh = scn->y1_min_thresh;
  if (scn->decoder) zbar_decoder_new_scan(scn->decoder);
  return (edge);
}
...
zbar_symbol_type_t zbar_scan_y(zbar_scanner_t *scn, int y) {
  ...
  y1_1 = y0_1 - y0_2;
  {
    ...
    if ((abs(y1_1) < abs(y1_2)) && ((y1_1 >= 0) == (y1_2 >= 0))) y1_1 = y1_2;
  }
  ...
  if ((!y2_1 || ((y2_1 > 0) ? y2_2 < 0 : y2_2 > 0)) &&
    ...
    if (y1_rev) /* intensity change reversal - finalize previous edge */
