...
static __inline int check_width(unsigned w0, unsigned w1) {
  unsigned dw0 = w0;
  w0 *= 8;
  w1 *= 8;
 ...
 */
static __inline signed char aux_end(zbar_decoder_t *dcode, unsigned char fwd) {
  signed char code, i;
...
static __inline signed char aux_start(zbar_decoder_t *dcode) {
  ...
  if (!E1) {
    ...
    if (!decode_e(e3, dcode->ean.s4, 7) && !decode_e(e4, dcode->ean.s4, 7)) {
...
static __inline signed char decode4(zbar_decoder_t *dcode) {
  ...
  code =
      ((decode_e(e1, dcode->ean.s4, 7) << 2) | decode_e(e2, dcode->ean.s4, 7));
  if (code < 0) return (-1);
  ...
  if ((1 << code) & 0x0660) {
    ...
    if (alt) code = ((code >> 1) & 3) | 0x10; /* compress code space */
...
static __inline char ean_part_end2(ean_decoder_t *ean, ean_pass_t *pass) {
  if (!TEST_CFG(ean->ean2_config, ZBAR_CFG_ENABLE)) return (ZBAR_NONE);

  /* extract parity bits */
  unsigned char par = ((pass->raw[1] & 0x10) >> 3 | (pass->raw[2] & 0x10) >> 4);
  /* calculate "checksum" */
  unsigned char chk = ~((pass->raw[1] & 0xf) * 10 + (pass->raw[2] & 0xf)) & 0x3;
  dbprintf(2, " par=%x chk=%x", par, chk);
  if (par != chk) return (ZBAR_NONE);
...
static __inline zbar_symbol_type_t ean_part_end4(ean_pass_t *pass,
                                                 unsigned char fwd) {
  /* extract parity bits */
  unsigned char par = ((pass->raw[1] & 0x10) >> 1 | (pass->raw[2] & 0x10) >> 2 |
                       (pass->raw[3] & 0x10) >> 3 | (pass->raw[4] & 0x10) >> 4);

  dbprintf(2, " par=%x", par);
  if (par && par != 0xf) /* invalid parity combination */
    return (ZBAR_NONE);
  ...
  if (!par) return (ZBAR_EAN8 | EAN_RIGHT);
  return (ZBAR_EAN8 | EAN_LEFT);
}

static __inline char ean_part_end5(ean_decoder_t *ean, ean_pass_t *pass) {
  if (!TEST_CFG(ean->ean5_config, ZBAR_CFG_ENABLE)) return (ZBAR_NONE);

  /* extract parity bits */
  unsigned char par = ((pass->raw[1] & 0x10) | (pass->raw[2] & 0x10) >> 1 |
                       (pass->raw[3] & 0x10) >> 2 | (pass->raw[4] & 0x10) >> 3 |
                       (pass->raw[5] & 0x10) >> 4);
  /* calculate checksum */
  unsigned char chk = (((pass->raw[1] & 0x0f) + (pass->raw[2] & 0x0f) * 3 +
  ...
  if (parchk != chk) return (ZBAR_NONE);
...
static __inline zbar_symbol_type_t ean_part_end7(ean_decoder_t *ean,
  ...
  if (par & 1) pass->raw[0] >>= 4;
  pass->raw[0] &= 0xf;
  dbprintf(2, " par=%02x(%x)", par, pass->raw[0]);

  if (pass->raw[0] == 0xf) /* invalid parity combination */
    return (ZBAR_NONE);
  ...
  if (TEST_CFG(ean->ean13_config, ZBAR_CFG_ENABLE)) {
    if (!par) return (ZBAR_EAN13 | EAN_RIGHT);
    if (par & 0x20) return (ZBAR_EAN13 | EAN_LEFT);
  }
  if (par && !(par & 0x20)) return (ZBAR_UPCE);
...
static __inline zbar_symbol_type_t decode_pass(zbar_decoder_t *dcode,
  ...
  if (get_color(dcode) == ZBAR_SPACE) {
    if (pass->state & STATE_ADDON) {
      ...
      if (part) dcode->ean.direction = (pass->state & STATE_REV) != 0;
      pass->state = -1;
      return (part);
  ...
  if (!(idx & 0x03) && idx <= 0x14) {
    ...
    if ((code < 0 && idx != 0x10) ||
      ...
      dbprintf(2, " raw=%d%d%d%d%d%d%d", pass->raw[0] & 0xf, pass->raw[1] & 0xf,
               pass->raw[2] & 0xf, pass->raw[3] & 0xf, pass->raw[4] & 0xf,
               pass->raw[5] & 0xf, pass->raw[6] & 0xf);
    }
  }
  return (0);
}

static __inline signed char ean_verify_checksum(ean_decoder_t *ean, int n) {
  unsigned char chk = 0;
  unsigned char i, d;
  for (i = 0; i < n; i++) {
...
static __inline unsigned char isbn10_calc_checksum(ean_decoder_t *ean) {
  ...
  if (!chk) return ('0');
  chk = 11 - chk;
  if (chk < 10) return (chk + '0');
  return ('X');
}

static __inline void ean_expand_upce(ean_decoder_t *ean, ean_pass_t *pass) {
  int i = 0;
...
static __inline zbar_symbol_type_t integrate_partial(ean_decoder_t *ean,
  ...
  {
    for (i = part; i > 0; i--) ean->buf[i - 1] = pass->raw[i] & 0xf;
    ean->left = part;
  } else
  ...
  if (part == ZBAR_EAN13) {
    ...
    if (!ean->buf[0] && TEST_CFG(ean->upca_config, ZBAR_CFG_ENABLE))
      ...
      if (ean->buf[2] == 8 && TEST_CFG(ean->isbn10_config, ZBAR_CFG_ENABLE))
        part = ZBAR_ISBN10;
    ...
    if (TEST_CFG(ean->upce_config, ZBAR_CFG_ENABLE)) {
      ...
      for (i = 2; i < 8; i++) ean->buf[i] = pass->raw[i - 1] & 0xf;
  ...
  dbprintf(2, " dir=%d %x/%x=%x", ean->direction, ean->left, ean->right, part);
  return (part);
}
