...
static __inline unsigned char code39_decode1(unsigned char enc, unsigned e,
                                             unsigned s) {
...
static __inline int code39_postprocess(zbar_decoder_t *dcode) {
  ...
  for (i = 0; i < dcode39->character; i++)
    dcode->buf[i] =
        ((dcode->buf[i] < 0x2b) ? code39_characters[(unsigned)dcode->buf[i]]
                                : '?');
...
zbar_symbol_type_t _zbar_decode_code39(zbar_decoder_t *dcode) {
  ...
  if (dcode39->character < 0) {
    if (get_color(dcode) != ZBAR_BAR) return (ZBAR_NONE);
    dbprintf(2, "      code39:");
    return (code39_decode_start(dcode));
  }

  if (++dcode39->element < 9) return (ZBAR_NONE);
