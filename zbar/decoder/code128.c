...
static __inline unsigned char calc_check(unsigned char c) {
  if (!(c & 0x80)) return (0x18);
  c &= 0x7f;
  if (c < 0x3d) return ((c < 0x30 && c != 0x17) ? 0x10 : 0x20);
  if (c < 0x50) return ((c == 0x4d) ? 0x20 : 0x10);
...
static __inline signed char decode6(zbar_decoder_t *dcode) {
  ...
  if (chk - 7 > bars || bars > chk + 7) return (-1);
...
static __inline unsigned char validate_checksum(zbar_decoder_t *dcode) {
  ...
  if (dcode128->character < 3) return (1);
  ...
  err = (sum != check);
  if (err) dbprintf(1, " [checksum error]\n");
  return (err);
}

/* expand and decode character set C */
static __inline unsigned postprocess_c(zbar_decoder_t *dcode, unsigned start,
...
zbar_symbol_type_t _zbar_decode_code128(zbar_decoder_t *dcode) {
  ...
  if (dcode128->character < 0) {
    ...
    if (dcode128->character > 1) release_lock(dcode, ZBAR_CODE128);
    dcode128->character = -1;
    return (0);
  } else {
    unsigned dw;
  ...
  zassert(dcode->buf_alloc > dcode128->character, 0,
          "alloc=%x idx=%x c=%02x %s\n", dcode->buf_alloc, dcode128->character,
          c, _zbar_decoder_buf_dump(dcode->buf, dcode->buf_alloc));
  ...
  if (dcode128->character > 2 &&
      ((dcode128->direction) ? c >= START_A && c <= START_C : c == STOP_FWD)) {
    ...
    dcode128->character = -1;
    if (!sym) release_lock(dcode, ZBAR_CODE128);
    return (sym);
  }
