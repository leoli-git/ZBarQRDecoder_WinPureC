...
static __inline unsigned char i25_decode1(unsigned char enc, unsigned e,
                                          unsigned s) {
...
static __inline signed char i25_decode_start(zbar_decoder_t *dcode) {
  i25_decoder_t *dcode25 = &dcode->i25;
  if (dcode25->s10 < 10) return (ZBAR_NONE);
...
static __inline int i25_acquire_lock(zbar_decoder_t *dcode) {
  ...
  /* copy holding buffer */
  for (i = 4; --i >= 0;) dcode->buf[i] = dcode->i25.buf[i];
  return (0);
}
...
static __inline signed char i25_decode_end(zbar_decoder_t *dcode) {
  ...
  if (dcode25->character <= 4 && i25_acquire_lock(dcode)) return (ZBAR_PARTIAL);
...
zbar_symbol_type_t _zbar_decode_i25(zbar_decoder_t *dcode) {
  ...
  if (dcode25->character < 0 && !i25_decode_start(dcode)) return (ZBAR_NONE);
...
reset:
  if (dcode25->character >= 4) release_lock(dcode, ZBAR_I25);
  dcode25->character = -1;
  return (ZBAR_NONE);
}
