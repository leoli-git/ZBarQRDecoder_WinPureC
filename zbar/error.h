...
static __inline int err_copy(void *dst_c, void *src_c) {
  errinfo_t *dst = dst_c;
  errinfo_t *src = src_c;
...
static __inline int err_capture(const void *container, errsev_t sev,
                                zbar_error_t type, const char *func,
                                const char *detail) {
  errinfo_t *err = (errinfo_t *)container;
  assert(err->magic == ERRINFO_MAGIC);
#ifdef HAVE_ERRNO_H
  if (type == ZBAR_ERR_SYSTEM) err->errnum = errno;
#endif
#ifdef _WIN32
  if (type == ZBAR_ERR_WINAPI) err->errnum = GetLastError();
#endif
  err->sev = sev;
  err->type = type;
  err->func = func;
  err->detail = detail;
  if (_zbar_verbosity >= 1) _zbar_error_spew(err, 0);
  return (-1);
}

static __inline int err_capture_str(const void *container, errsev_t sev,
                                    zbar_error_t type, const char *func,
                                    const char *detail, const char *arg) {
  errinfo_t *err = (errinfo_t *)container;
  assert(err->magic == ERRINFO_MAGIC);
  if (err->arg_str) zbar_free(err->arg_str);
  err->arg_str = strdup(arg);
  return (err_capture(container, sev, type, func, detail));
}

static __inline int err_capture_int(const void *container, errsev_t sev,
                                    zbar_error_t type, const char *func,
...
static __inline int err_capture_num(const void *container, errsev_t sev,
                                    zbar_error_t type, const char *func,
...
static __inline void err_init(errinfo_t *err, errmodule_t module) {
