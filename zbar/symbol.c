...
int _zbar_get_symbol_hash(zbar_symbol_type_t sym) {
  static const signed char hash[0x20] = {
      0x00, 0x01, 0x10, 0x11, -1,   0x11, 0x16, 0x0c, 0x05, 0x06, 0x08,
      -1,   0x04, 0x03, 0x07, 0x12, -1,   -1,   -1,   -1,   -1,   -1,
      -1,   0x02, -1,   0x00, 0x12, 0x0c, 0x0b, 0x1d, 0x0a, 0x00,
  };
...
void _zbar_symbol_free(zbar_symbol_t *sym) {
  ...
  if (sym->pts) zbar_free(sym->pts);
  if (sym->data_alloc && sym->data) zbar_free(sym->data);
  zbar_free(sym);
}
...
char *zbar_symbol_xml(const zbar_symbol_t *sym, char **buf, unsigned *len) {
  ...
  if (binary) datalen = (sym->datalen + 2) / 3 * 4 + sym->datalen / 57 + 3;

  maxlen = (MAX_STATIC + strlen(type) + strlen(orient) + datalen +
            MAX_INT_DIGITS + 1);
  unsigned int mods = sym->modifiers;
  if (mods) maxlen += MAX_MOD;
  unsigned int cfgs = sym->configs & ~(1 << ZBAR_CFG_ENABLE);
  if (cfgs) maxlen += MAX_CFG;
  if (binary) maxlen += MAX_INT_DIGITS;

  if (!*buf || (*len < maxlen)) {
    if (*buf) zbar_free(*buf);
  ...
  if (mods) {
    ...
    for (j = 0; mods && j < ZBAR_MOD_NUM; j++, mods >>= 1)
      if (mods & 1) TMPL_FMT("%s ", zbar_get_modifier_name(j));
  ...
  if (cfgs) {
    ...
    for (j = 0; cfgs && j < ZBAR_CFG_NUM; j++, cfgs >>= 1)
      if (cfgs & 1) TMPL_FMT("%s ", zbar_get_config_name(j));
    /* cleanup trailing space */
    n--;
    TMPL_COPY("'");
  }

  if (sym->cache_count) TMPL_FMT(" count='%d'", sym->cache_count);

  TMPL_COPY("><data");
  if (binary) TMPL_FMT(" format='base64' length='%d'", sym->datalen);
  TMPL_COPY("><![CDATA[");
