...
static int rs_quadratic_solve(const rs_gf256 *_gf, unsigned _b, unsigned _c,
  ...
  l2 = rs_hgmul(_gf,
                rs_gmul(_gf, z2, z2) ^ rs_hgmul(_gf, z2, 255 - (255 / 15)) ^ c0,
                2 * (255 / 15));
...
static void rs_poly_mul_x(unsigned char *_p, const unsigned char *_q,
                          int _dp1) {
...
/*Divide the polynomial by the free variable, x (shift the coefficients).
  The number of coefficients, _dp1, must be non-zero.*/
static void rs_poly_div_x(unsigned char *_p, const unsigned char *_q,
                          int _dp1) {
...
static void rs_calc_syndrome(const rs_gf256 *_gf, int _m0, unsigned char *_s,
                             int _npar, const unsigned char *_data,
                             int _ndata) {
  int i;
  int j;
  for (j = 0; j < _npar; j++) {
...
static int rs_find_roots(const rs_gf256 *_gf, unsigned char *_epos,
                         const unsigned char *_lambda, int _nerrors,
                         int _ndata) {
  unsigned alpha;
  int nroots;
  int i;
  ...
  for (i = 0; i < _npar; i++)
    if (s[i]) {
      ...
      /*If we can't locate any errors, we can't force the syndrome values to
        ...
        Conversely, if we have too many errors, there's no reason to even
        attempt the root search.*/
    ...
    for (b = 0; b < 256; b++)
      for (c = 0; c < 256; c++)
        for (d = 0; d < 256; d++) {
          ...
          /*We only care about missing roots if the quartic had 4 distinct,
             non-zero roots.*/
          if (mroots == 4)
            for (j = 0; j < mroots; j++) {
              for (i = 0; i < nroots; i++)
