...
void _zbar_image_scanner_recycle_syms(zbar_image_scanner_t *iscn,
  ...
  for (; sym; sym = next) {
    ...
    if (sym->refcnt && _zbar_refcnt(&sym->refcnt, -1)) {
      ...
      if (sym->syms) {
        if (_zbar_refcnt(&sym->syms->refcnt, -1)) assert(0);
...
static __inline int recycle_syms(zbar_image_scanner_t *iscn,
  ...
  if (_zbar_refcnt(&syms->refcnt, -1)) return (1);
...
__inline zbar_symbol_t *_zbar_image_scanner_alloc_sym(
    zbar_image_scanner_t *iscn, zbar_symbol_type_t type, int datalen) {
  /* recycle old or alloc new symbol */
  zbar_symbol_t *sym = NULL;
  int i;
  for (i = 0; i < RECYCLE_BUCKETS - 1; i++)
    if (datalen <= 1 << (i * 2)) break;
  ...
  if (datalen > 0) {
    ...
    if (sym->data_alloc < datalen) {
      if (sym->data) zbar_free(sym->data);
...
static __inline void cache_sym(zbar_image_scanner_t *iscn, zbar_symbol_t *sym) {
...
static void symbol_handler(zbar_decoder_t *dcode) {
  ...
  for (sym = iscn->syms->head; sym; sym = sym->next)
    if (sym->type == type && sym->datalen == datalen &&
      ...
      if (TEST_CFG(iscn, ZBAR_CFG_POSITION)) /* add new point to existing set */
  ...
  if (dir) sym->orient = (iscn->dy != 0) + ((iscn->du ^ dir) & 2);
...
void zbar_image_scanner_destroy(zbar_image_scanner_t *iscn) {
  ...
  if (iscn->scn) zbar_scanner_destroy(iscn->scn);
  iscn->scn = NULL;
  if (iscn->dcode) zbar_decoder_destroy(iscn->dcode);
  iscn->dcode = NULL;
  for (i = 0; i < RECYCLE_BUCKETS; i++) {
...
zbar_image_data_handler_t *zbar_image_scanner_set_data_handler(
    zbar_image_scanner_t *iscn, zbar_image_data_handler_t *handler,
    const void *userdata) {
...
int zbar_image_scanner_set_config(zbar_image_scanner_t *iscn,
  ...
  if (cfg < ZBAR_CFG_POSITION) {
    int c, i;
    if (cfg > ZBAR_CFG_UNCERTAINTY) return (1);
    c = cfg - ZBAR_CFG_UNCERTAINTY;
    if (sym > ZBAR_PARTIAL) {
      i = _zbar_get_symbol_hash(sym);
      iscn->sym_configs[c][i] = val;
    } else
      for (i = 0; i < NUM_SYMS; i++) iscn->sym_configs[c][i] = val;
    return (0);
  }

  if (sym > ZBAR_PARTIAL) return (1);
  ...
  if (cfg > ZBAR_CFG_POSITION) return (1);
...
const zbar_symbol_set_t *zbar_image_scanner_get_results(
    const zbar_image_scanner_t *iscn) {
  return (iscn->syms);
}
...
int zbar_scan_image(zbar_image_scanner_t *iscn, zbar_image_t *img) {
  ...
  if (density > 0) {
    ...
    if (border > img->crop_h / 2) border = img->crop_h / 2;
    border += img->crop_y;
    assert(border <= h);
  ...
  if (density > 0) {
    ...
    if (border > img->crop_w / 2) border = img->crop_w / 2;
    border += img->crop_x;
    assert(border <= w);
  ...
  if (syms->nsyms && iscn->handler) iscn->handler(img, iscn->userdata);
