...
static __inline void sym_add_point(zbar_symbol_t *sym, int x, int y) {
  int i = sym->npts;
...
static __inline void _zbar_symbol_refcnt(zbar_symbol_t *sym, int delta) {
  if (!_zbar_refcnt(&sym->refcnt, delta) && delta <= 0) _zbar_symbol_free(sym);
}
